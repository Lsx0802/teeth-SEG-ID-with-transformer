...
if args.comet:
    ...
    experiment.log_asset('ICT/utils/data_augmentation.py')
    experiment.log_asset('ICT/utils/parser.py')
else:
...
if args.weighted_ce:
    weights = torch.from_numpy(numpy.load('ICT/losses/ce_weights.npy')).to(dtype=torch.float32, device=device)
...
for fold, (train_ids, test_ids) in enumerate(kfold.split(train_dataset)):
    ...
    #MODEL INIT
    # SOTA architectures
    if args.model_name == "Trans":
        model = Trans(spatial_dims=3, in_channels=1, out_channels=args.classes, img_size=args.patch_size,
