...
test_crop_transform = Compose(
            [
                ...
                ToDeviceD(keys=keys, device=device),
                EnsureTypeD(keys=keys, data_type="tensor", device=device),
                SpacingD(keys=keys, pixdim=(0.6,0.6,0.6), mode=("bilinear", "nearest")),
...
"""weight path"""
checkpoint_path='cbctseg_model_state_dict.pt'
""""""
...
model_defor = torch.nn.DataParallel(model, device_ids=device_ids)
...
# datalist = []
# nifti_paths_scans=natsorted(os.path.join(data_root_dir,'000.nii.gz'))
# nifti_paths_labels=natsorted(os.path.join(data_root_dir,'000_mask.nii.gz'))
# nifti_list = [{'image' : scan, 'label': label} for scan, label in zip(nifti_paths_scans, nifti_paths_labels)]
# datalist.extend(nifti_list)
...
with torch.no_grad():
    for idx, test_data in enumerate(tqdm.tqdm(test_loader)):
        ...
        if model_name == "SwinUNETR":
            test_data["pred"] = sliding_window_inference(test_data["image"], (128,128,128), sw_batch_size=8, predictor=model, overlap=0.25, sw_device=device,
        ...
        output_postprocessed = post_processing_segmentation(model_output, instance_ground_truth=gt_image, device=device, distance_treshold=0.2, 
                                                            opening_kernel=3, instance_min_volume=1000)
        ...
        if calculate_metrics:
            ...
            for func in metrics_list:
                ...
                func.reset()
            metrics=np.array(metrics_values)*[1,1,1,0.4,0.4,0.4] #pidim 0.6 mm/px
            # print(metrics)
...
if calculate_metrics:
    ...
    metrics_df.to_csv(f'csv_files/inference_results_{model_name}_{ablation}.csv', sep='\t', index=False)
