...
# nifti_list = [{'image' : scan, 'label': label} for scan, label in zip(nifti_paths_scans, nifti_paths_labels)]
nifti_list = [{'image' : scan} for scan in zip(nifti_paths_scans, nifti_paths_labels)]
datalist.extend(nifti_list)
...
with torch.no_grad():
    for idx, test_data in enumerate(tqdm.tqdm(test_loader)):
        ...
        #                                                     opening_kernel=3, instance_min_volume=600)
        output_postprocessed = post_processing_segmentation(model_output, instance_ground_truth=None, device=device, distance_treshold=0.2,
                                                           opening_kernel=3, instance_min_volume=600)
        ...
        if calculate_metrics:
            ...
            for func in metrics_list:
                ...
                func.reset()
            metrics=np.array(metrics_values)*[1,1,1,0.6,0.6,0.6] #pidim 0.6mm/px
            # print(metrics)
